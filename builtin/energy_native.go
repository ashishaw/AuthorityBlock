...
func init() {
	defines := []struct {
		...
		{"native_get", func(env *xenv.Environment) []interface{} {
			...
			bal, err := Energy.Native(env.State(), env.BlockContext().Time).Get(ablock.Address(addr))
			if err != nil {
				panic(err)
			}
		...
		{"native_add", func(env *xenv.Environment) []interface{} {
			...
			exist, err := env.State().Exists(ablock.Address(args.Addr))
			if err != nil {
				panic(err)
			}
			if exist {
			...
			if err := Energy.Native(env.State(), env.BlockContext().Time).Add(ablock.Address(args.Addr), args.Amount); err != nil {
				panic(err)
			}
			return nil
		}},
		{"native_sub", func(env *xenv.Environment) []interface{} {
			...
			env.UseGas(ablock.GetBalanceGas)
			ok, err := Energy.Native(env.State(), env.BlockContext().Time).Sub(ablock.Address(args.Addr), args.Amount)
			if err != nil {
				panic(err)
			}
			if ok {
		...
		{"native_master", func(env *xenv.Environment) []interface{} {
			...
			env.UseGas(ablock.GetBalanceGas)
			master, err := env.State().GetMaster(ablock.Address(addr))
			if err != nil {
				panic(err)
			}
