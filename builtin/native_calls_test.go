...
func (c *ccase) To(to ablock.Address) *ccase {
	c.to = to
	return c
}

func (c *ccase) Caller(caller ablock.Address) *ccase {
	c.caller = caller
	return c
}
...
func (c *ccase) TxID(txID ablock.Bytes32) *ccase {
	c.txID = txID
	return c
}
...
func (c *ccase) GasPayer(gasPayer ablock.Address) *ccase {
	c.gasPayer = gasPayer
	return c
}
...
func TestParamsNative(t *testing.T) {
	executor := ablock.BytesToAddress([]byte("e"))
	db := muxdb.NewMem()
	b0 := buildGenesis(db, func(state *state.State) error {
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(ablock.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		return nil
	})
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{}, ablock.NoFork)
...
func TestAuthorityNative(t *testing.T) {
	var (
		master1   = ablock.BytesToAddress([]byte("master1"))
		endorsor1 = ablock.BytesToAddress([]byte("endorsor1"))
		identity1 = ablock.BytesToBytes32([]byte("identity1"))

		master2   = ablock.BytesToAddress([]byte("master2"))
		endorsor2 = ablock.BytesToAddress([]byte("endorsor2"))
		identity2 = ablock.BytesToBytes32([]byte("identity2"))

		master3   = ablock.BytesToAddress([]byte("master3"))
		endorsor3 = ablock.BytesToAddress([]byte("endorsor3"))
		identity3 = ablock.BytesToBytes32([]byte("identity3"))
		executor  = ablock.BytesToAddress([]byte("e"))
	)
	...
	b0 := buildGenesis(db, func(state *state.State) error {
		...
		state.SetBalance(ablock.Address(endorsor1), ablock.InitialProposerEndorsement)
		state.SetCode(builtin.Params.Address, builtin.Params.RuntimeBytecodes())
		builtin.Params.Native(state).Set(ablock.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
		builtin.Params.Native(state).Set(ablock.KeyProposerEndorsement, ablock.InitialProposerEndorsement)
		return nil
	})
	repo, _ := chain.NewRepository(db, b0)
	st := state.New(db, b0.Header().StateRoot(), 0, 0, 0)
	...
	candidateEvent := func(nodeMaster ablock.Address, action string) *tx.Event {
		...
		return &tx.Event{
			...
			Topics:  []ablock.Bytes32{ev.ID(), ablock.BytesToBytes32(nodeMaster[:])},
			Data:    data,
		}
	}
	...
	test.Case("revoke", master2).
		Caller(ablock.BytesToAddress([]byte("some one"))).
		Assert(t)
...
func TestEnergyNative(t *testing.T) {
	var (
		addr   = ablock.BytesToAddress([]byte("addr"))
		to     = ablock.BytesToAddress([]byte("to"))
		master = ablock.BytesToAddress([]byte("master"))
		eng    = big.NewInt(1000)
	)
	...
	transferEvent := func(from, to ablock.Address, value *big.Int) *tx.Event {
		ev, _ := builtin.Energy.ABI.EventByName("Transfer")
		data, _ := ev.Encode(value)
		return &tx.Event{
			Address: builtin.Energy.Address,
			Topics:  []ablock.Bytes32{ev.ID(), ablock.BytesToBytes32(from[:]), ablock.BytesToBytes32(to[:])},
			Data:    data,
		}
	}
	approvalEvent := func(owner, spender ablock.Address, value *big.Int) *tx.Event {
		ev, _ := builtin.Energy.ABI.EventByName("Approval")
		data, _ := ev.Encode(value)
		return &tx.Event{
			Address: builtin.Energy.Address,
			Topics:  []ablock.Bytes32{ev.ID(), ablock.BytesToBytes32(owner[:]), ablock.BytesToBytes32(spender[:])},
			Data:    data,
		}
	}

	rt := runtime.New(chain, st, &xenv.BlockContext{Time: b0.Header().Timestamp()}, ablock.NoFork)
	test := &ctest{
		rt:     rt,
	...
	test.Case("transferFrom", addr, ablock.BytesToAddress([]byte("some one")), big.NewInt(10)).
		Caller(to).
		ShouldLog(transferEvent(addr, ablock.BytesToAddress([]byte("some one")), big.NewInt(10))).
		ShouldOutput(true).
		Assert(t)
...
func TestPrototypeNative(t *testing.T) {
	var (
		acc1 = ablock.BytesToAddress([]byte("acc1"))
		acc2 = ablock.BytesToAddress([]byte("acc2"))

		master    = ablock.BytesToAddress([]byte("master"))
		notmaster = ablock.BytesToAddress([]byte("notmaster"))
		user      = ablock.BytesToAddress([]byte("user"))
		notuser   = ablock.BytesToAddress([]byte("notuser"))

		credit       = big.NewInt(1000)
		recoveryRate = big.NewInt(10)
		sponsor      = ablock.BytesToAddress([]byte("sponsor"))
		notsponsor   = ablock.BytesToAddress([]byte("notsponsor"))

		key      = ablock.BytesToBytes32([]byte("account-key"))
		value    = ablock.BytesToBytes32([]byte("account-value"))
		contract ablock.Address
	)
	...
	st.SetStorage(ablock.Address(acc1), key, value)
	st.SetBalance(ablock.Address(acc1), big.NewInt(1))

	masterEvent := func(self, newMaster ablock.Address) *tx.Event {
		ev, _ := builtin.Prototype.Events().EventByName("$Master")
		data, _ := ev.Encode(newMaster)
		return &tx.Event{
			Address: self,
			Topics:  []ablock.Bytes32{ev.ID()},
			Data:    data,
		}
	}

	creditPlanEvent := func(self ablock.Address, credit, recoveryRate *big.Int) *tx.Event {
	...
	userEvent := func(self, user ablock.Address, action string) *tx.Event {
		...
		return &tx.Event{
			Address: self,
			Topics:  []ablock.Bytes32{ev.ID(), ablock.BytesToBytes32(user.Bytes())},
			Data:    data,
		}
	}

	sponsorEvent := func(self, sponsor ablock.Address, action string) *tx.Event {
		...
		return &tx.Event{
			Address: self,
			Topics:  []ablock.Bytes32{ev.ID(), ablock.BytesToBytes32(sponsor.Bytes())},
			Data:    data,
		}
	}
	...
	test.Case("storageFor", acc1, key).
		ShouldOutput(value).
		Assert(t)
	test.Case("storageFor", acc1, ablock.BytesToBytes32([]byte("some-key"))).
		ShouldOutput(ablock.Bytes32{}).
		Assert(t)

	// should be hash of rlp raw
	expected, err := st.GetStorage(builtin.Prototype.Address, ablock.Blake2b(contract.Bytes(), []byte("credit-plan")))
	assert.Nil(t, err)
	test.Case("storageFor", builtin.Prototype.Address, ablock.Blake2b(contract.Bytes(), []byte("credit-plan"))).
...
func TestPrototypeNativeWithLongerBlockNumber(t *testing.T) {
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{
		Number: ablock.MaxStateHistory + 1,
...
func TestExtensionNative(t *testing.T) {
	...
	gasPayer := ablock.BytesToAddress([]byte("gasPayer"))
	...
	rt := runtime.New(chain, st, &xenv.BlockContext{Number: 2, Time: b2.Header().Timestamp(), TotalScore: b2.Header().TotalScore(), Signer: b2_singer}, ablock.NoFork)
	...
	test.Case("blake2b256", []byte("hello world")).
		ShouldOutput(ablock.Blake2b([]byte("hello world"))).
		Assert(t)
	...
	test.Case("txID").
		TxID(ablock.BytesToBytes32([]byte("txID"))).
		ShouldOutput(ablock.BytesToBytes32([]byte("txID"))).
		Assert(t)
