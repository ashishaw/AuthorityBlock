...
var (
	headKey = ablock.Blake2b([]byte("head"))
	tailKey = ablock.Blake2b([]byte("tail"))
)
...
func New(addr ablock.Address, state *state.State) *Authority {
	return &Authority{addr, state}
}

func (a *Authority) getEntry(nodeMaster ablock.Address) (*entry, error) {
	var entry entry
	if err := a.state.DecodeStorage(a.addr, ablock.BytesToBytes32(nodeMaster[:]), func(raw []byte) error {
		if len(raw) == 0 {
			return nil
		}
...
func (a *Authority) setEntry(nodeMaster ablock.Address, entry *entry) error {
	return a.state.EncodeStorage(a.addr, ablock.BytesToBytes32(nodeMaster[:]), func() ([]byte, error) {
		if entry.IsEmpty() {
			return nil, nil
		}
...
func (a *Authority) getAddressPtr(key ablock.Bytes32) (addr *ablock.Address, err error) {
...
func (a *Authority) setAddressPtr(key ablock.Bytes32, addr *ablock.Address) error {
...
func (a *Authority) Get(nodeMaster ablock.Address) (listed bool, endorsor ablock.Address, identity ablock.Bytes32, active bool, err error) {
	var entry *entry
...
func (a *Authority) Add(nodeMaster ablock.Address, endorsor ablock.Address, identity ablock.Bytes32) (bool, error) {
...
func (a *Authority) Revoke(nodeMaster ablock.Address) (bool, error) {
	entry, err := a.getEntry(nodeMaster)
	if err != nil {
		return false, err
	}
...
func (a *Authority) Update(nodeMaster ablock.Address, active bool) (bool, error) {
	entry, err := a.getEntry(nodeMaster)
	if err != nil {
		return false, err
	}
...
func (a *Authority) First() (*ablock.Address, error) {
	return a.getAddressPtr(headKey)
}

// Next returns address of next node master address after given node master address.
func (a *Authority) Next(nodeMaster ablock.Address) (*ablock.Address, error) {
	entry, err := a.getEntry(nodeMaster)
	if err != nil {
		return nil, err
	}
