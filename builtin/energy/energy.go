...
var (
	initialSupplyKey = ablock.Blake2b([]byte("initial-supply"))
	totalAddSubKey   = ablock.Blake2b([]byte("total-add-sub"))
)
...
func New(addr ablock.Address, state *state.State, blockTime uint64) *Energy {
...
func (e *Energy) Get(addr ablock.Address) (*big.Int, error) {
	return e.state.GetEnergy(addr, e.blockTime)
}

// Add add amount of energy to given address.
func (e *Energy) Add(addr ablock.Address, amount *big.Int) error {
	if amount.Sign() == 0 {
		return nil
	}
...
func (e *Energy) Sub(addr ablock.Address, amount *big.Int) (bool, error) {
	if amount.Sign() == 0 {
		return true, nil
	}
	eng, err := e.state.GetEnergy(addr, e.blockTime)
