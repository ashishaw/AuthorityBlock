...
type callFrame struct {
	...
	From    ablock.Address          `json:"from"`
	To      ablock.Address          `json:"to,omitempty"`
...
type account struct {
	...
	Storage map[common.Hash]ablock.Bytes32 `json:"storage"`
}
...
func RunTracerTest(t *testing.T, data *traceTest, tracerName string) json.RawMessage {
	...
	for addr, account := range data.State {
		st.SetBalance(ablock.Address(addr), (*big.Int)(account.Balance))
		if len(account.Code) > 0 {
			st.SetCode(ablock.Address(addr), account.Code)
		}
		for k, v := range account.Storage {
			st.SetStorage(ablock.Address(addr), ablock.Bytes32(k), v)
		}
	}

	rt := runtime.New(chain, st, &xenv.BlockContext{
		Number: data.Context.BlockNumber,
	}, ablock.GetForkConfig(gene.Header().ID()))
