...
import (
	...
	"github.com/ashishaw/authorityblock/p2psrv/rpc"
...
func (p *Peer) Head() (id ablock.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
...
func (p *Peer) UpdateHead(id ablock.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
...
func (p *Peer) MarkTransaction(hash ablock.Bytes32) {
	// that's 10~100 block intervals
	expiration := mclock.AbsTime(time.Second * time.Duration(ablock.BlockInterval*uint64(rand.Intn(91)+10)))
...
func (p *Peer) MarkBlock(id ablock.Bytes32) {
	p.knownBlocks.Add(id, struct{}{})
}

// IsTransactionKnown returns if the transaction is known.
func (p *Peer) IsTransactionKnown(hash ablock.Bytes32) bool {
...
func (p *Peer) IsBlockKnown(id ablock.Bytes32) bool {
