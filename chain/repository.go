...
func NewRepository(db *muxdb.MuxDB, genesis *block.Block) (*Repository, error) {
	...
	if val, err := repo.props.Get(bestBlockIDKey); err != nil {
		...
		bestID := ablock.BytesToBytes32(val)
	...
	if val, err := repo.props.Get(steadyBlockIDKey); err != nil {
		...
		repo.steadyID.Store(ablock.BytesToBytes32(val))
	}
	return repo, nil
}
...
func (r *Repository) SetBestBlockID(id ablock.Bytes32) (err error) {
	defer func() {
		if err == nil {
			r.tick.Broadcast()
		}
...
func (r *Repository) SteadyBlockID() ablock.Bytes32 {
	return r.steadyID.Load().(ablock.Bytes32)
}

// SetSteadyBlockID set the given block id as the head block id of the steady chain.
func (r *Repository) SetSteadyBlockID(id ablock.Bytes32) error {
	prev := r.steadyID.Load().(ablock.Bytes32)
...
func (r *Repository) saveBlock(block *block.Block, receipts tx.Receipts, conflicts, steadyNum uint32) (*BlockSummary, error) {
	var (
		...
		summary     = BlockSummary{header, []ablock.Bytes32{}, uint64(block.Size()), conflicts, steadyNum}
...
func (r *Repository) AddBlock(newBlock *block.Block, receipts tx.Receipts, conflicts uint32) error {
	...
	newSteadyID := r.steadyID.Load().(ablock.Bytes32)
...
func (r *Repository) ScanHeads(from uint32) ([]ablock.Bytes32, error) {
	var start [4]byte
	...
	heads := make([]ablock.Bytes32, 0, 16)

	for ok := iter.Last(); ok; ok = iter.Prev() {
		heads = append(heads, ablock.BytesToBytes32(iter.Key()))
	}
...
func (r *Repository) GetBlockSummary(id ablock.Bytes32) (summary *BlockSummary, err error) {
...
func (r *Repository) GetBlockTransactions(id ablock.Bytes32) (tx.Transactions, error) {
	summary, err := r.GetBlockSummary(id)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetBlock(id ablock.Bytes32) (*block.Block, error) {
	summary, err := r.GetBlockSummary(id)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetBlockReceipts(id ablock.Bytes32) (tx.Receipts, error) {
	summary, err := r.GetBlockSummary(id)
	if err != nil {
		return nil, err
	}
