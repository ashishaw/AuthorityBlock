...
type Chain struct {
	repo     *Repository
	headID   ablock.Bytes32
	lazyInit func() (*muxdb.Trie, error)
}

func newChain(repo *Repository, headID ablock.Bytes32) *Chain {
	var (
...
func (c *Chain) GenesisID() ablock.Bytes32 {
	return c.repo.GenesisBlock().Header().ID()
}

// HeadID returns the head block id.
func (c *Chain) HeadID() ablock.Bytes32 {
	return c.headID
}

// GetBlockID returns block id by given block number.
func (c *Chain) GetBlockID(num uint32) (ablock.Bytes32, error) {
	trie, err := c.lazyInit()
	if err != nil {
	...
	if len(data) == 0 {
		return ablock.Bytes32{}, errNotFound
	}
	return ablock.BytesToBytes32(data), nil
}

// GetTransactionMeta returns tx meta by given tx id.
func (c *Chain) GetTransactionMeta(id ablock.Bytes32) (*TxMeta, error) {
	...
	for iter.Next() {
		...
		blockID := ablock.BytesToBytes32(iter.Key()[32:])
...
func (c *Chain) GetTransaction(id ablock.Bytes32) (*tx.Transaction, *TxMeta, error) {
...
func (c *Chain) GetTransactionReceipt(txID ablock.Bytes32) (*tx.Receipt, error) {
...
func (c *Chain) HasBlock(id ablock.Bytes32) (bool, error) {
...
func (c *Chain) Exclude(other *Chain) ([]ablock.Bytes32, error) {
	oHeadID := other.headID
	oHeadNum := block.Number(oHeadID)
	var ids []ablock.Bytes32
...
func (r *Repository) NewChain(headID ablock.Bytes32) *Chain {
	return newChain(r, headID)
}

func (r *Repository) indexBlock(parentConflicts uint32, newBlockID ablock.Bytes32, newConflicts uint32) error {
	var (
