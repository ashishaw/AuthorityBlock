...
func TestScanHeads(t *testing.T) {
	...
	assert.Equal(t, []ablock.Bytes32{repo.GenesisBlock().Header().ID()}, heads)
	...
	heads, err = repo.ScanHeads(0)
	assert.Nil(t, err)
	assert.Equal(t, []ablock.Bytes32{b1.Header().ID()}, heads)
	...
	heads, err = repo.ScanHeads(0)
	assert.Nil(t, err)
	assert.Equal(t, []ablock.Bytes32{b2.Header().ID()}, heads)
	...
	if heads[0] == b2.Header().ID() {
		assert.Equal(t, []ablock.Bytes32{b2.Header().ID(), b2x.Header().ID()}, heads)
	} else {
		assert.Equal(t, []ablock.Bytes32{b2x.Header().ID(), b2.Header().ID()}, heads)
	}
	...
	heads, err = repo.ScanHeads(0)
	assert.Nil(t, err)
	assert.Equal(t, []ablock.Bytes32{b3.Header().ID(), b2x.Header().ID()}, heads)
	...
	assert.Nil(t, err)
	assert.Equal(t, []ablock.Bytes32{b3.Header().ID()}, heads)
	...
	if heads[0] == b3.Header().ID() {
		assert.Equal(t, []ablock.Bytes32{b3.Header().ID(), b3x.Header().ID(), b2x.Header().ID()}, heads)
	} else {
		assert.Equal(t, []ablock.Bytes32{b3x.Header().ID(), b3.Header().ID(), b2x.Header().ID()}, heads)
	}
}
