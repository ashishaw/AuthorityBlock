...
func TestChain(t *testing.T) {
	...
	_, err = c.GetTransactionMeta(ablock.Bytes32{})
	assert.True(t, c.IsNotFound(err))
	...
	assert.Equal(t, M([]ablock.Bytes32{b3.Header().ID()}, nil), M(c1.Exclude(c2)))
	assert.Equal(t, M([]ablock.Bytes32{b3x.Header().ID()}, nil), M(c2.Exclude(c1)))

	dangleID := ablock.Bytes32{0, 0, 0, 4}
