...
func DevAccounts() []DevAccount {
	...
	for _, str := range privKeys {
		...
		accs = append(accs, DevAccount{ablock.Address(addr), pk})
	}
...
func NewDevnet() *Genesis {
	...
	builder := new(Builder).
		...
		Call(
			...
			executor).
		Call(
			tx.NewClause(&builtin.Authority.Address).WithData(mustEncodeInput(builtin.Authority.ABI, "add", soloBlockSigner.Address, soloBlockSigner.Address, ablock.BytesToBytes32([]byte("Solo Block Signer")))),
			executor)
