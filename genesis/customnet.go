...
type CustomGenesis struct {
	...
	ForkConfig *ablock.ForkConfig `json:"forkConfig"`
}
...
func NewCustomNet(gen *CustomGenesis) (*Genesis, error) {
	...
	if gen.GasLimit == 0 {
		gen.GasLimit = ablock.InitialGasLimit
	}
	var executor ablock.Address
			...
			for _, a := range gen.Accounts {
				...
				if len(a.Storage) > 0 {
					for k, v := range a.Storage {
						state.SetStorage(a.Address, ablock.MustParseBytes32(k), v)
					}
				}
			}
	...
	if bgp != nil {
		...
		bgp = ablock.InitialBaseGasPrice
	}
	...
	if e != nil {
		...
		e = ablock.InitialProposerEndorsement
	}

	data := mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))
	builder.Call(tx.NewClause(&builtin.Params.Address).WithData(data), ablock.Address{})

	data = mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyRewardRatio, r)
	builder.Call(tx.NewClause(&builtin.Params.Address).WithData(data), executor)

	data = mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyBaseGasPrice, bgp)
	builder.Call(tx.NewClause(&builtin.Params.Address).WithData(data), executor)

	data = mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyProposerEndorsement, e)
	builder.Call(tx.NewClause(&builtin.Params.Address).WithData(data), executor)
	...
	if m := gen.Params.MaxBlockProposers; m != nil {
		...
		data = mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyMaxBlockProposers, new(big.Int).SetUint64(*m))
		builder.Call(tx.NewClause(&builtin.Params.Address).WithData(data), executor)
	}
...
type Account struct {
	...
	Storage map[string]ablock.Bytes32 `json:"storage"`
}

// Authority is the authority node info
type Authority struct {
	MasterAddress   ablock.Address `json:"masterAddress"`
	EndorsorAddress ablock.Address `json:"endorsorAddress"`
	Identity        ablock.Bytes32 `json:"identity"`
}
...
type Params struct {
	...
	ExecutorAddress     *ablock.Address    `json:"executorAddress"`
