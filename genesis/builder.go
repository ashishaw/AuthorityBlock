...
func (b *Builder) Call(clause *tx.Clause, caller ablock.Address) *Builder {
...
func (b *Builder) ForkConfig(fc ablock.ForkConfig) *Builder {
	b.forkConfig = fc
	return b
}

// ComputeID compute genesis ID.
func (b *Builder) ComputeID() (ablock.Bytes32, error) {
	db := muxdb.NewMem()
...
func (b *Builder) Build(stater *state.Stater) (blk *block.Block, events tx.Events, transfers tx.Transfers, err error) {
	state := stater.NewState(ablock.Bytes32{}, 0, 0, 0)
	...
	parentID := ablock.Bytes32{0xff, 0xff, 0xff, 0xff} //so, genesis number is 0
