...
func NewTestnet() *Genesis {
	...
	executor := ablock.MustParseAddress("0xB5A34b62b63A6f1EE99DFD30b133B657859f8d79")
	acccount0 := ablock.MustParseAddress("0xe59D475Abe695c7f67a8a2321f33A856B0B4c71d")

	master0 := ablock.MustParseAddress("0x25AE0ef84dA4a76D5a1DFE80D3789C2c46FeE30a")
	endorser0 := ablock.MustParseAddress("0xb4094c25f86d628fdD571Afc4077f0d0196afB48")
	...
	builder := new(Builder).
		...
		Call(
			tx.NewClause(&builtin.Params.Address).WithData(mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyExecutorAddress, new(big.Int).SetBytes(executor[:]))),
			ablock.Address{}).
		Call(
			tx.NewClause(&builtin.Params.Address).WithData(mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyRewardRatio, ablock.InitialRewardRatio)),
			executor).
		Call(
			tx.NewClause(&builtin.Params.Address).WithData(mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyBaseGasPrice, ablock.InitialBaseGasPrice)),
			executor).
		Call(
			tx.NewClause(&builtin.Params.Address).WithData(mustEncodeInput(builtin.Params.ABI, "set", ablock.KeyProposerEndorsement, ablock.InitialProposerEndorsement)),
			executor).
		// add master0 as the initial block proposer
		Call(tx.NewClause(&builtin.Authority.Address).WithData(mustEncodeInput(builtin.Authority.ABI, "add", master0, endorser0, ablock.BytesToBytes32([]byte("master0")))),
			executor)
