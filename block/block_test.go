...
func TestBlock(t *testing.T) {
	tx1 := new(tx.Builder).Clause(tx.NewClause(&ablock.Address{})).Clause(tx.NewClause(&ablock.Address{})).Build()
	...
	var (
		...
		emptyRoot   ablock.Bytes32 = ablock.BytesToBytes32([]byte("0"))
		beneficiary ablock.Address = ablock.BytesToAddress([]byte("abc"))
	)
