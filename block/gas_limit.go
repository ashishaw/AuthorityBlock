...
func (gl GasLimit) IsValid(parentGasLimit uint64) bool {
	gasLimit := uint64(gl)
	if gasLimit < ablock.MinGasLimit {
		return false
	}
	var diff uint64
	...
	return diff <= parentGasLimit/ablock.GasLimitBoundDivisor
}
...
func (gl GasLimit) Qualify(parentGasLimit uint64) uint64 {
	gasLimit := uint64(gl)
	maxDiff := parentGasLimit / ablock.GasLimitBoundDivisor
	if gasLimit > parentGasLimit {
...
func (gl GasLimit) Adjust(delta int64) uint64 {
	gasLimit := uint64(gl)
	maxDiff := gasLimit / ablock.GasLimitBoundDivisor
	...
	// reduce
	diff := min64(uint64(-delta), maxDiff)
	if ablock.MinGasLimit+diff > gasLimit {
		// reach floor
		return ablock.MinGasLimit
	}
	return gasLimit - diff
}
