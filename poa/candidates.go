...
import (
	...
	"github.com/ashishaw/authorityblock/builtin/authority"
...
type Candidates struct {
	...
	masters    map[ablock.Address]int  // map master address to list index
	endorsors  map[ablock.Address]bool // endorsor bitset
	satisfied  []int
	referenced bool
}
...
func NewCandidates(list []*authority.Candidate) *Candidates {
	masters := make(map[ablock.Address]int)
	endorsors := make(map[ablock.Address]bool)
...
func (c *Candidates) Pick(state *state.State) ([]Proposer, error) {
	...
	if len(satisfied) == 0 {
		// re-pick
		endorsement, err := builtin.Params.Native(state).Get(ablock.KeyProposerEndorsement)
		if err != nil {
			return nil, err
		}

		mbp, err := builtin.Params.Native(state).Get(ablock.KeyMaxBlockProposers)
		if err != nil {
			return nil, err
		}
		maxBlockProposers := mbp.Uint64()
		if maxBlockProposers == 0 || maxBlockProposers > ablock.InitialMaxBlockProposers {
			maxBlockProposers = ablock.InitialMaxBlockProposers
		}
...
func (c *Candidates) Update(addr ablock.Address, active bool) bool {
...
func (c *Candidates) IsEndorsor(addr ablock.Address) bool {
