...
func TestNewSchedulerV2(t *testing.T) {
	seed := ablock.Bytes32{}.Bytes()
	parentNumber := uint32(10)

	/*
		var ordered []ablock.Address
		var list []struct {
			addr ablock.Address
			hash ablock.Bytes32
		}
		var addrs = []ablock.Address{p1, p2, p3, p4, p5}
		var num [4]byte
		...
		for _, addr := range addrs {
			list = append(list, struct {
				addr ablock.Address
				hash ablock.Bytes32
			}{addr, ablock.Blake2b(seed, num[:], addr.Bytes())})
		}
	...
	*/
	...
	tests := []struct {
	...
	}{
		{
			...
			&SchedulerV2{Proposer{p1, true}, parentTime, []ablock.Address{p1, p4, p3, p2, p5}},
			false,
		},
		{
			...
			&SchedulerV2{Proposer{p1, false}, parentTime, []ablock.Address{p1, p4, p3, p2, p5}},
			false,
		},
		{
