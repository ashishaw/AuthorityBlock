...
func NewSchedulerV2(
	...
	for _, p := range proposers {
		...
		if p.Active || p.Address == addr {
			list = append(list, struct {
				...
				p.Address,
				ablock.Blake2b(seed, num[:], p.Address.Bytes()),
			})
		}
	}
	...
	shuffled := make([]ablock.Address, 0, len(list))
	for _, t := range list {
...
func (s *SchedulerV2) IsScheduled(blockTime uint64, proposer ablock.Address) bool {
