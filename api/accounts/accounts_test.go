...
import (
	...
	ABI "github.com/ashishaw/authorityblock/abi"
...
var addr = ablock.BytesToAddress([]byte("to"))
var value = big.NewInt(10000)
var storageKey = ablock.Bytes32{}
var storageValue = byte(1)

var contractAddr ablock.Address
...
func getStorage(t *testing.T) {
	...
	h, err := ablock.ParseBytes32(value["value"])
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, ablock.BytesToBytes32([]byte{storageValue}), h, "storage should be equal")
	assert.Equal(t, http.StatusOK, statusCode, "OK")
}
...
func initAccountServer(t *testing.T) {
	...
	contractAddr = ablock.CreateContractAddress(transaction.ID(), 1, 0)
	...
	accounts.New(repo, stater, math.MaxUint64, ablock.NoFork).Mount(router, "/accounts")
...
func packTx(repo *chain.Repository, stater *state.Stater, transaction *tx.Transaction, t *testing.T) {
	packer := packer.New(repo, stater, genesis.DevAccounts()[0].Address, &genesis.DevAccounts()[0].Address, ablock.NoFork)
