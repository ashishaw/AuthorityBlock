...
func (a *Accounts) getCode(addr ablock.Address, summary *chain.BlockSummary) ([]byte, error) {
	code, err := a.stater.
...
func (a *Accounts) handleGetCode(w http.ResponseWriter, req *http.Request) error {
	...
	addr, err := ablock.ParseAddress(hexAddr)
	if err != nil {
...
func (a *Accounts) getAccount(addr ablock.Address, summary *chain.BlockSummary) (*Account, error) {
...
func (a *Accounts) getStorage(addr ablock.Address, key ablock.Bytes32, summary *chain.BlockSummary) (ablock.Bytes32, error) {
...
func (a *Accounts) handleGetAccount(w http.ResponseWriter, req *http.Request) error {
	addr, err := ablock.ParseAddress(mux.Vars(req)["address"])
	if err != nil {
		return utils.BadRequest(errors.WithMessage(err, "address"))
	}
	summary, err := a.handleRevision(req.URL.Query().Get("revision"))
...
func (a *Accounts) handleGetStorage(w http.ResponseWriter, req *http.Request) error {
	...
	key, err := ablock.ParseBytes32(mux.Vars(req)["key"])
	if err != nil {
...
func (a *Accounts) handleCallContract(w http.ResponseWriter, req *http.Request) error {
	...
	if mux.Vars(req)["address"] != "" {
		address, err := ablock.ParseAddress(mux.Vars(req)["address"])
		if err != nil {
			return utils.BadRequest(errors.WithMessage(err, "address"))
		}
		addr = &address
...
func (a *Accounts) handleRevision(revision string) (*chain.BlockSummary, error) {
	...
	if len(revision) == 66 || len(revision) == 64 {
		blockID, err := ablock.ParseBytes32(revision)
		if err != nil {
