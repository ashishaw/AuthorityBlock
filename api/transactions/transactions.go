...
func (t *Transactions) getRawTransaction(txID ablock.Bytes32, head ablock.Bytes32, allowPending bool) (*rawTransaction, error) {
...
func (t *Transactions) getTransactionByID(txID ablock.Bytes32, head ablock.Bytes32, allowPending bool) (*Transaction, error) {
	chain := t.repo.NewChain(head)
	tx, meta, err := chain.GetTransaction(txID)
	if err != nil {
		if t.repo.IsNotFound(err) {
...
func (t *Transactions) getTransactionReceiptByID(txID ablock.Bytes32, head ablock.Bytes32) (*Receipt, error) {
	chain := t.repo.NewChain(head)
	tx, meta, err := chain.GetTransaction(txID)
	if err != nil {
		if t.repo.IsNotFound(err) {
...
func (t *Transactions) handleGetTransactionByID(w http.ResponseWriter, req *http.Request) error {
	id := mux.Vars(req)["id"]
	txID, err := ablock.ParseBytes32(id)
	if err != nil {
...
func (t *Transactions) parseHead(head string) (ablock.Bytes32, error) {
	if head == "" {
		return t.repo.BestBlockSummary().Header.ID(), nil
	}
	h, err := ablock.ParseBytes32(head)
	if err != nil {
