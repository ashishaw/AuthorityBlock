...
type Flow struct {
	...
	processedTxs map[ablock.Bytes32]bool // txID -> reverted
	gasUsed      uint64
	txs          tx.Transactions
...
func newFlow(
	...
	return &Flow{
		...
		runtime:      runtime,
		processedTxs: make(map[ablock.Bytes32]bool),
		features:     features,
	}
}
...
func (f *Flow) findTx(txID ablock.Bytes32) (found bool, reverted bool, err error) {
...
func (f *Flow) Adopt(tx *tx.Transaction) error {
	origin, _ := tx.Origin()
	if f.Number() >= f.packer.forkConfig.BLOCKLIST && ablock.IsOriginBlocked(origin) {
	...
	case f.gasUsed+tx.Gas() > f.runtime.Context().GasLimit:
		...
		if f.gasUsed+ablock.TxGas+ablock.ClauseGas <= f.runtime.Context().GasLimit {
...
func (f *Flow) Pack(privateKey *ecdsa.PrivateKey, newBlockConflicts uint32, shouldVote bool) (*block.Block, *state.Stage, tx.Receipts, error) {
	if f.packer.nodeMaster != ablock.Address(crypto.PubkeyToAddress(privateKey.PublicKey)) {
