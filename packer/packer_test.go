...
func (ti *txIterator) OnProcessed(txID ablock.Bytes32, err error) {
}
...
func TestP(t *testing.T) {
	...
	for {
		...
		p := packer.New(repo, stater, a1.Address, &a1.Address, ablock.NoFork)
		...
		_, _, err = consensus.New(repo, stater, ablock.NoFork).Process(best, blk, uint64(time.Now().Unix()*2), 0)
		assert.Nil(t, err)
...
func TestForkVIP191(t *testing.T) {
	...
	b0, _, _, err := new(genesis.Builder).
		...
		State(func(state *state.State) error {
			...
			builtin.Authority.Native(state).Add(a1.Address, a1.Address, ablock.BytesToBytes32([]byte{}))
			return nil
		}).
		Build(stater)
...
func TestBlocklist(t *testing.T) {
	...
	ablock.MockBlocklist([]string{a0.Address.String()})
