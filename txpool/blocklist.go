...
func (bl *blocklist) Contains(addr ablock.Address) bool {
	bl.lock.Lock()
...
func (bl *blocklist) readList(r io.Reader) (map[ablock.Address]bool, error) {
	scanner := bufio.NewScanner(r)
	list := make(map[ablock.Address]bool)
	...
	for scanner.Scan() {
		...
		addr, err := ablock.ParseAddress(addrStr)
		if err != nil {
			return nil, err
		}
		list[addr] = true
