...
func TestInsert(t *testing.T) {
	...
	exp, _ := ablock.ParseBytes32("6ca394ff9b13d6690a51dea30b1b5c43108e52944d30b9095227c49bae03ff8b")
	root := trie.Hash()
	if root != exp {
	...
	exp, _ = ablock.ParseBytes32("e9d7f23f40cd82fe35f5a7a6778c3503f775f3623ba7a71fb335f0eee29dac8a")
...
func TestDelete(t *testing.T) {
	...
	hash := trie.Hash()
	exp, _ := ablock.ParseBytes32("79a9b42da0e261b9f3ca9e78560ac8d486bcce2da8a5ddb2df8721d4c0dc2d0a")
	if hash != exp {
...
func runRandTest(rt randTest) bool {
	...
	for i, step := range rt {
		...
		case opItercheckhash:
			checktr, _ := New(ablock.Bytes32{}, nil)
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		_, tmpdb := tempDB()
		trie, _ = New(ablock.Bytes32{}, tmpdb)
	}
	k := make([]byte, 32)
...
func BenchmarkHash(b *testing.B) {
	...
	trie := newEmpty()
	for i := 0; i < len(addresses); i++ {
		trie.Update(ablock.Blake2b(addresses[i][:]).Bytes(), accounts[i])
	}
	b.ResetTimer()
	b.ReportAllocs()
	trie.Hash()
...
func TestExtended(t *testing.T) {
	db := ethdb.NewMemDatabase()
	tr := NewExtended(ablock.Bytes32{}, 0, db, false)
	...
	for _, v := range vals1 {
		tr.Update([]byte(v.k), []byte(v.v), ablock.Blake2b([]byte(v.v)).Bytes())
	}
	...
	for _, v := range vals1 {
		...
		if string(meta) != string(ablock.Blake2b(val).Bytes()) {
...
func TestNonCryptoExtended(t *testing.T) {
	...
	tr := NewExtended(ablock.Bytes32{}, 0, db, true)
	var root ablock.Bytes32
	n := uint32(100)
	for i := uint32(0); i < n; i++ {
		var k [4]byte
		binary.BigEndian.PutUint32(k[:], i)
		tr.Update(k[:], ablock.Blake2b(k[:]).Bytes(), nil)
	...
	for i := uint32(0); i < n; i++ {
		...
		val, _, err := tr.Get(k[:])
		assert.Nil(t, err)
		assert.Equal(t, ablock.Blake2b(k[:]).Bytes(), val)
	}
}
