...
type nodeIteratorState struct {
	hash    ablock.Bytes32 // Hash of the node being iterated (nil if not standalone)
	node    node         // Trie node being iterated
	parent  ablock.Bytes32 // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() ablock.Bytes32 {
	if len(it.stack) == 0 {
		return ablock.Bytes32{}
	}
...
func (it *nodeIterator) Parent() ablock.Bytes32 {
	if len(it.stack) == 0 {
		return ablock.Bytes32{}
	}
...
func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor ablock.Bytes32) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() ablock.Bytes32 {
	return it.b.Hash()
}
...
func (it *differenceIterator) Parent() ablock.Bytes32 {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == ablock.Bytes32{}
...
func (it *unionIterator) Hash() ablock.Bytes32 {
...
func (it *unionIterator) Parent() ablock.Bytes32 {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == ablock.Bytes32{}) {
			it.count++
