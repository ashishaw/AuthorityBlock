		...
		if hash, ok := hn.(*hashNode); ok || i == 0 {
			...
			if fromLevel > 0 {
				...
				if ok {
					...
					proofDb.Put(ablock.Blake2b(hasher.tmp).Bytes(), hasher.tmp)
				}
			}
		}
	}
...
// wrong value.
func VerifyProof(rootHash ablock.Bytes32, key []byte, proofDb DatabaseReader) (value []byte, err error, nodes int) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
