...
var NonCryptoNodeHash = ablock.BytesToBytes32(bytes.Repeat([]byte{0xff}, 32))
...
const hashLen = len(ablock.Bytes32{})
...
func decodeRef(buf []byte, trailing *trailing, cacheGen uint16) (node, []byte, error) {
	...
	if valLen == 32 {
		return &hashNode{Hash: ablock.BytesToBytes32(val), seq: seq}, rest, nil
	}
