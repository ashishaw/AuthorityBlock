...
type Output struct {
	...
	ContractAddress *ablock.Address // if create a new contract, or is nil.
}
...
// Runtime bases on EVM and Authority Block builtins.
type Runtime struct {
	vmConfig    vm.Config
	chain       *chain.Chain
	state       *state.State
...
func New(
	...
	if forkConfig.ETH_IST == ctx.Number {
		for addr := range vm.PrecompiledContractsIstanbul {
			if err := state.SetCode(ablock.Address(addr), EmptyRuntimeBytecode); err != nil {
				panic(err)
			}
		}
...
func (rt *Runtime) newEVM(stateDB *statedb.StateDB, clauseIndex uint32, txCtx *xenv.TransactionContext) *vm.EVM {
	...
	return vm.NewEVM(vm.Context{
		...
		Transfer: func(_ vm.StateDB, sender, recipient common.Address, amount *big.Int) {
			...
			senderEnergy, err := rt.state.GetEnergy(ablock.Address(sender), rt.ctx.Time)
			if err != nil {
				panic(err)
			}
			recipientEnergy, err := rt.state.GetEnergy(ablock.Address(recipient), rt.ctx.Time)
			if err != nil {
				panic(err)
			}

			if err := rt.state.SetEnergy(ablock.Address(sender), senderEnergy, rt.ctx.Time); err != nil {
				panic(err)
			}
			if err := rt.state.SetEnergy(ablock.Address(recipient), recipientEnergy, rt.ctx.Time); err != nil {
				panic(err)
			}
			...
			stateDB.AddTransfer(&tx.Transfer{
				Sender:    ablock.Address(sender),
				Recipient: ablock.Address(recipient),
		...
		NewContractAddress: func(_ *vm.EVM, counter uint32) common.Address {
			return common.Address(ablock.CreateContractAddress(txCtx.ID, clauseIndex, counter))
		},
		InterceptContractCall: func(evm *vm.EVM, contract *vm.Contract, readonly bool) ([]byte, error, bool) {
			...
			abi, run, found := builtin.FindNativeCall(ablock.Address(contract.Address()), contract.Input)
			if !found {
				lastNonNativeCallGas = contract.Gas
				return nil, nil, false
			}
		...
		OnCreateContract: func(_ *vm.EVM, contractAddr, caller common.Address) {
			...
			if err := rt.state.SetMaster(ablock.Address(contractAddr), ablock.Address(caller)); err != nil {
				panic(err)
			}
		...
		OnSuicideContract: func(_ *vm.EVM, contractAddr, tokenReceiver common.Address) {
			...
			amount, err := rt.state.GetEnergy(ablock.Address(contractAddr), rt.ctx.Time)
			if err != nil {
				panic(err)
			}
			if amount.Sign() != 0 {
				...
				receiverEnergy, err := rt.state.GetEnergy(ablock.Address(tokenReceiver), rt.ctx.Time)
				if err != nil {
					panic(err)
				}
				if err := rt.state.SetEnergy(
					ablock.Address(tokenReceiver),
			...
			if amount := stateDB.GetBalance(contractAddr); amount.Sign() != 0 {
				...
				stateDB.AddTransfer(&tx.Transfer{
					Sender:    ablock.Address(contractAddr),
					Recipient: ablock.Address(tokenReceiver),
					Amount:    amount,
				})
			}
		},
...
func (rt *Runtime) PrepareClause(
	...
	exec = func() (output *Output, interrupted bool, err error) {
		...
		if clause.To() == nil {
			...
			contractAddr = (*ablock.Address)(&caddr)
		} else {
...
func (rt *Runtime) PrepareTransaction(tx *tx.Transaction) (*TransactionExecutor, error) {
	...
	return &TransactionExecutor{
		...
		Finalize: func() (*Tx.Receipt, error) {
			...
			// reward
			rewardRatio, err := builtin.Params.Native(rt.state).Get(ablock.KeyRewardRatio)
			if err != nil {
				return nil, err
			}
