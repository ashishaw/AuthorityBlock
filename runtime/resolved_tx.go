...
type ResolvedTransaction struct {
	tx           *tx.Transaction
	Origin       ablock.Address
	Delegator    *ablock.Address
	IntrinsicGas uint64
	Clauses      []*tx.Clause
}
...
func (r *ResolvedTransaction) CommonTo() *ablock.Address {
	if len(r.Clauses) == 0 {
		return nil
	}
...
func (r *ResolvedTransaction) BuyGas(state *state.State, blockTime uint64) (
	...
	gasPrice *big.Int,
	payer ablock.Address,
	returnGas func(uint64) error,
	err error,
) {
	if baseGasPrice, err = builtin.Params.Native(state).Get(ablock.KeyBaseGasPrice); err != nil {
		return
	}
	...
	if r.Delegator != nil {
		...
		return nil, nil, ablock.Address{}, nil, errors.New("insufficient energy")
	}
...
func (r *ResolvedTransaction) ToContext(
	...
	blockNumber uint32,
	getBlockID func(uint32) (ablock.Bytes32, error),
