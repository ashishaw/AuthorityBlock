...
func TestContractSuicide(t *testing.T) {
	...
	addr := ablock.BytesToAddress([]byte("acc01"))
	...
	exec, _ := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{Time: time}, ablock.NoFork).
	...
	expectedEvent := &tx.Event{
		...
		Topics:  []ablock.Bytes32{event.ID(), ablock.BytesToBytes32(addr.Bytes()), ablock.BytesToBytes32(origin.Bytes())},
...
func TestChainID(t *testing.T) {
	...
	exec, _ := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{}, ablock.ForkConfig{ETH_IST: 0}).
		PrepareClause(tx.NewClause(&addr).WithData(methodData), 0, math.MaxUint64, &xenv.TransactionContext{})
	out, _, err := exec()
	assert.Nil(t, err)
	assert.Nil(t, out.VMErr)

	assert.Equal(t, g.ID(), ablock.BytesToBytes32(out.Data))
}
...
func TestBlake2(t *testing.T) {
	...
	assert.Equal(t, ablock.MustParseBytes32("ba80a53f981c4d0d6a2797b69f12f6e94c212f14685ac4b74b12bb6fdbffa2d1"), ablock.Bytes32(hashes[0]))
	assert.Equal(t, ablock.MustParseBytes32("7d87c5392aab792dc252d5de4533cc9518d38aa8dbf1925ab92386edd4009923"), ablock.Bytes32(hashes[1]))
}
...
func TestCall(t *testing.T) {
	...
	rt := runtime.New(repo.NewChain(b0.Header().ID()), state, &xenv.BlockContext{}, ablock.NoFork)
	...
	assert.Equal(t, ablock.Address(addr), genesis.DevAccounts()[0].Address)
...
func TestExecuteTransaction(t *testing.T) {
	...
	// addr1 := ablock.Address(crypto.PubkeyToAddress(key.PublicKey))
	// addr2 := ablock.BytesToAddress([]byte("acc2"))
	...
	// 	ablock.Address{}, 0, 0, 0, func(uint32) ablock.Bytes32 { return ablock.Bytes32{} })
