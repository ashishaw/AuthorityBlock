...
import (
	...
	"github.com/ashishaw/authorityblock/lowrlp"
	"github.com/ashishaw/authorityblock/muxdb"
	"github.com/ashishaw/authorityblock/stackedmap"
...
type State struct {
	...
	cache          map[ablock.Address]*cachedObject // cache of accounts trie
	sm             *stackedmap.StackedMap         // keeps revisions of accounts state
	steadyBlockNum uint32
}

// New create state object.
func New(db *muxdb.MuxDB, root ablock.Bytes32, blockNum, blockConflicts, steadyBlockNum uint32) *State {
	state := State{
		db:             db,
		trie:           db.NewTrie(AccountTrieName, root, blockNum, blockConflicts),
		cache:          make(map[ablock.Address]*cachedObject),
...
func (s *State) Checkout(root ablock.Bytes32, blockNum, blockConflicts, steadyBlockNum uint32) *State {
...
func (s *State) cacheGetter(key interface{}) (value interface{}, exist bool, err error) {
	switch k := key.(type) {
	case ablock.Address: // get account
	...
	case codeKey: // get code
		obj, err := s.getCachedObject(ablock.Address(k))
		if err != nil {
			return nil, false, err
		}
...
func (s *State) getCachedObject(addr ablock.Address) (*cachedObject, error) {
...
func (s *State) getAccount(addr ablock.Address) (*Account, error) {
...
func (s *State) getAccountCopy(addr ablock.Address) (Account, error) {
...
func (s *State) updateAccount(addr ablock.Address, acc *Account) {
	s.sm.Put(addr, acc)
}

func (s *State) getStorageBarrier(addr ablock.Address) int {
	b, _, _ := s.sm.Get(storageBarrierKey(addr))
	return b.(int)
}

func (s *State) setStorageBarrier(addr ablock.Address, barrier int) {
	s.sm.Put(storageBarrierKey(addr), barrier)
}

// GetBalance returns balance for the given address.
func (s *State) GetBalance(addr ablock.Address) (*big.Int, error) {
	acc, err := s.getAccount(addr)
	if err != nil {
...
func (s *State) SetBalance(addr ablock.Address, balance *big.Int) error {
...
func (s *State) GetEnergy(addr ablock.Address, blockTime uint64) (*big.Int, error) {
	acc, err := s.getAccount(addr)
	if err != nil {
		return nil, &Error{err}
	}
...
func (s *State) SetEnergy(addr ablock.Address, energy *big.Int, blockTime uint64) error {
	cpy, err := s.getAccountCopy(addr)
	if err != nil {
		return &Error{err}
	}
...
func (s *State) GetMaster(addr ablock.Address) (ablock.Address, error) {
	acc, err := s.getAccount(addr)
	if err != nil {
		return ablock.Address{}, &Error{err}
	}
	return ablock.BytesToAddress(acc.Master), nil
}

// SetMaster set master for the given address.
func (s *State) SetMaster(addr ablock.Address, master ablock.Address) error {
	cpy, err := s.getAccountCopy(addr)
	if err != nil {
		return &Error{err}
	}
...
func (s *State) GetStorage(addr ablock.Address, key ablock.Bytes32) (ablock.Bytes32, error) {
	raw, err := s.GetRawStorage(addr, key)
	if err != nil {
		return ablock.Bytes32{}, &Error{err}
	}
	if len(raw) == 0 {
		return ablock.Bytes32{}, nil
	}
	...
	if kind == rlp.List {
		...
		return ablock.Blake2b(raw), nil
	}
	return ablock.BytesToBytes32(content), nil
}

// SetStorage set storage value for the given address and key.
func (s *State) SetStorage(addr ablock.Address, key, value ablock.Bytes32) {
	if value.IsZero() {
...
func (s *State) GetRawStorage(addr ablock.Address, key ablock.Bytes32) (rlp.RawValue, error) {
...
func (s *State) SetRawStorage(addr ablock.Address, key ablock.Bytes32, raw rlp.RawValue) {
...
func (s *State) EncodeStorage(addr ablock.Address, key ablock.Bytes32, enc func() ([]byte, error)) error {
	raw, err := enc()
	if err != nil {
		return &Error{err}
	}
...
func (s *State) DecodeStorage(addr ablock.Address, key ablock.Bytes32, dec func([]byte) error) error {
	raw, err := s.GetRawStorage(addr, key)
	if err != nil {
		return &Error{err}
	}
...
func (s *State) GetCode(addr ablock.Address) ([]byte, error) {
...
func (s *State) GetCodeHash(addr ablock.Address) (ablock.Bytes32, error) {
	acc, err := s.getAccount(addr)
	if err != nil {
		return ablock.Bytes32{}, &Error{err}
	}
	return ablock.BytesToBytes32(acc.CodeHash), nil
}

// SetCode set code for the given address.
func (s *State) SetCode(addr ablock.Address, code []byte) error {
	var codeHash []byte
	if len(code) > 0 {
...
// See Account.IsEmpty()
func (s *State) Exists(addr ablock.Address) (bool, error) {
	acc, err := s.getAccount(addr)
	if err != nil {
...
func (s *State) Delete(addr ablock.Address) {
	s.sm.Put(codeKey(addr), []byte(nil))
...
func (s *State) BuildStorageTrie(addr ablock.Address) (trie *muxdb.Trie, err error) {
	acc, err := s.getAccount(addr)
	if err != nil {
		return nil, &Error{err}
	}
	...
	if len(acc.StorageRoot) > 0 {
		...
		trie = s.db.NewTrie(
			...
			ablock.BytesToBytes32(acc.StorageRoot),
...
func (s *State) Stage(newBlockNum, newBlockConflicts uint32) (*Stage, error) {
	...
	var (
		changes = make(map[ablock.Address]*changed)
		codes   = make(map[ablock.Bytes32][]byte)

		storageTrieCreationCount uint64
	)

	// get or create changed account
	getChanged := func(addr ablock.Address) (*changed, error) {
	...
	s.sm.Journal(func(k, v interface{}) bool {
		...
		case codeKey:
			code := v.([]byte)
			if len(code) > 0 {
				codes[ablock.Bytes32(crypto.Keccak256Hash(code))] = code
			}
		case storageKey:
			...
			if c.storage == nil {
				c.storage = make(map[ablock.Bytes32]rlp.RawValue)
			}
		...
		case storageBarrierKey:
			if c, jerr = getChanged(ablock.Address(key)); jerr != nil {
				return false
			}
			...
			if len(c.storage) > 0 {
				...
				if c.baseStorageTrie != nil {
					...
					sTrie = s.db.NewTrie(
						...
						ablock.BytesToBytes32(c.data.StorageRoot),
...
type (
	...
	codeKey           ablock.Address
	storageBarrierKey ablock.Address
)
