...
func TestStage(t *testing.T) {
	...
	addr := ablock.BytesToAddress([]byte("acc1"))

	balance := big.NewInt(10)
	code := []byte{1, 2, 3}

	storage := map[ablock.Bytes32]ablock.Bytes32{
		ablock.BytesToBytes32([]byte("s1")): ablock.BytesToBytes32([]byte("v1")),
		ablock.BytesToBytes32([]byte("s2")): ablock.BytesToBytes32([]byte("v2")),
		ablock.BytesToBytes32([]byte("s3")): ablock.BytesToBytes32([]byte("v3"))}
	...
	assert.Equal(t, M(ablock.Bytes32(crypto.Keccak256Hash(code)), nil), M(state.GetCodeHash(addr)))
