...
// Account is the Ablock consensus representation of an account.
...
func (a *Account) CalcEnergy(blockTime uint64) *big.Int {
	...
	x.Mul(x, a.Balance)
	x.Mul(x, ablock.EnergyGrowthRate)
	x.Div(x, bigE18)
...
func loadAccount(trie *muxdb.Trie, addr ablock.Address, steadyBlockNum uint32) (*Account, *AccountMetadata, error) {
	hashedKey := ablock.Blake2b(addr[:])
...
func saveAccount(trie *muxdb.Trie, addr ablock.Address, a *Account, am *AccountMetadata) error {
	if a.IsEmpty() {
		hashedKey := ablock.Blake2b(addr[:])
...
func loadStorage(trie *muxdb.Trie, key ablock.Bytes32, steadyBlockNum uint32) (rlp.RawValue, error) {
	hashedKey := ablock.Blake2b(key[:])
...
func saveStorage(trie *muxdb.Trie, key ablock.Bytes32, data rlp.RawValue) error {
	hashedKey := ablock.Blake2b(key[:])
	return trie.Update(
		hashedKey[:],
		data,
