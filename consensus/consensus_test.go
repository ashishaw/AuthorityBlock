...
import (
	...
	"github.com/ashishaw/authorityblock/vrf"
)

func txBuilder(tag byte) *tx.Builder {
	address := ablock.BytesToAddress([]byte("addr"))
...
func newTestConsensus() (*testConsensus, error) {
	...
	gen := new(genesis.Builder).
		...
		State(func(state *state.State) error {
			...
			builtin.Params.Native(state).Set(ablock.KeyExecutorAddress, new(big.Int).SetBytes(genesis.DevAccounts()[0].Address[:]))
			for _, acc := range genesis.DevAccounts() {
				state.SetBalance(acc.Address, bal)
				state.SetEnergy(acc.Address, bal, launchTime)
				builtin.Authority.Native(state).Add(acc.Address, acc.Address, ablock.Bytes32{})
			}
			return nil
		})
	...
	flow, err := p.Schedule(parentSum, uint64(parent.Header().Timestamp()+100*ablock.BlockInterval))
	if err != nil {
		return nil, err
	}
	...
	flow2, err := p2.Schedule(b1sum, uint64(b1.Header().Timestamp()+100*ablock.BlockInterval))
	if err != nil {
		return nil, err
	}
	...
	tests := []struct {
		...
		},
		{
			"ErrFutureBlock", func(t *testing.T) {
				builder := tc.builder(tc.original.Header())
				blk, err := tc.sign(builder.Timestamp(tc.time + ablock.BlockInterval*2))
				if err != nil {
					t.Fatal(err)
				}
				err = tc.consent(blk)
	...
	tests := []struct {
		...
		},
		{
			"TxOriginBlocked", func(t *testing.T) {
				ablock.MockBlocklist([]string{genesis.DevAccounts()[9].Address.String()})
			...
			"ErrSignerInvalid", func(t *testing.T) {
				...
				expected := consensusError(
					fmt.Sprintf(
						...
						ablock.Address(crypto.PubkeyToAddress(pk.PublicKey)),
					),
				)
				assert.Equal(t, expected, err)
			},
			...
			"ErrTimestampUnscheduled", func(t *testing.T) {
				...
				expected := consensusError(
					fmt.Sprintf(
						...
						blk.Header().Timestamp(),
						ablock.Address(crypto.PubkeyToAddress(genesis.DevAccounts()[3].PrivateKey.PublicKey)),
					),
				)
				assert.Equal(t, expected, err)
			},
