...
type justifier struct {
	...
	votes    map[ablock.Address]bool
	comVotes uint64
}

func (engine *BFTEngine) newJustifier(parentID ablock.Bytes32) (*justifier, error) {
	...
	if absRound := blockNum/ablock.CheckpointInterval - engine.forkConfig.FINALITY/ablock.CheckpointInterval; absRound == 0 {
		parentQuality = 0
	} else {
		var err error
	...
	return &justifier{
		votes:         make(map[ablock.Address]bool),
...
func (js *justifier) AddBlock(blockID ablock.Bytes32, signer ablock.Address, isCOM bool) {
