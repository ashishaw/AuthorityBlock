...
import (
	...
	"github.com/ashishaw/authorityblock/cache"
...
func NewEngine(repo *chain.Repository, mainDB *muxdb.MuxDB, forkConfig ablock.ForkConfig, master ablock.Address) (*BFTEngine, error) {
	engine := BFTEngine{
		repo:       repo,
	...
	if val, err := engine.data.Get(finalizedKey); err != nil {
		...
		engine.finalized.Store(ablock.BytesToBytes32(val))
	}
...
func (engine *BFTEngine) Finalized() ablock.Bytes32 {
	return engine.finalized.Load().(ablock.Bytes32)
}

// Accepts checks if the given block is on the same branch of finalized checkpoint.
func (engine *BFTEngine) Accepts(parentID ablock.Bytes32) (bool, error) {
...
// Packer only.
func (engine *BFTEngine) ShouldVote(parentID ablock.Bytes32) (bool, error) {
	// laze init casts
	if engine.casts == nil {
...
func (engine *BFTEngine) findCheckpointByQuality(target uint32, finalized, parentID ablock.Bytes32) (blockID ablock.Bytes32, err error) {
	defer func() {
		if e := recover(); e != nil {
			err = e.(error)
			return
	...
	get := func(i int) (uint32, error) {
		id, err := c.GetBlockID(getStorePoint(searchStart + uint32(i)*ablock.CheckpointInterval))
		if err != nil {
			return 0, err
		}
	...
	n := int((block.Number(parentID) + 1 - searchStart) / ablock.CheckpointInterval)
	...
	if num == n {
		return ablock.Bytes32{}, errors.New("failed find the block by quality")
	}

	quality, err := get(num)
	if err != nil {
		return ablock.Bytes32{}, err
	}

	if quality != target {
		return ablock.Bytes32{}, errors.New("failed to find the block by quality")
	}

	return c.GetBlockID(searchStart + uint32(num)*ablock.CheckpointInterval)
}
...
func (engine *BFTEngine) getMaxBlockProposers(sum *chain.BlockSummary) (uint64, error) {
	...
	params, err := builtin.Params.Native(state).Get(ablock.KeyMaxBlockProposers)
	if err != nil {
		return 0, err
	}
	mbp := params.Uint64()
	if mbp == 0 || mbp > ablock.InitialMaxBlockProposers {
		mbp = ablock.InitialMaxBlockProposers
	}

	return mbp, nil
}

func (engine *BFTEngine) getQuality(id ablock.Bytes32) (quality uint32, err error) {
...
func getCheckPoint(blockNum uint32) uint32 {
	return blockNum / ablock.CheckpointInterval * ablock.CheckpointInterval
}
...
func getStorePoint(blockNum uint32) uint32 {
	return getCheckPoint(blockNum) + ablock.CheckpointInterval - 1
}
