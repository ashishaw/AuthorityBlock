...
import (
	...
	"github.com/ashishaw/authorityblock/cmd/ablock/bandwidth"
...
func (n *Node) houseKeeping(ctx context.Context) {
	...
	futureTicker := time.NewTicker(time.Duration(ablock.BlockInterval) * time.Second)
...
func (n *Node) writeLogs(newBlock *block.Block, newReceipts tx.Receipts, oldBestBlockID ablock.Bytes32) (err error) {
	var w *logdb.Writer
...
func (n *Node) processFork(newBlock *block.Block, oldBestBlockID ablock.Bytes32) {
	oldTrunk := n.repo.NewChain(oldBestBlockID)
	newTrunk := n.repo.NewChain(newBlock.Header().ParentID())
...
func checkClockOffset() {
	...
	if resp.ClockOffset > time.Duration(ablock.BlockInterval)*time.Second/2 {
