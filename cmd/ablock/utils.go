...
import (
	...
	"github.com/ashishaw/authorityblock/cmd/ablock/node"
	...
	"github.com/ashishaw/authorityblock/p2psrv"
...
func defaultConfigDir() string {
	if home := homeDir(); home != "" {
		return filepath.Join(home, ".org.ashishaw.ablock")
	}
	return ""
}
...
func defaultDataDir() string {
	...
	if home := homeDir(); home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "Application Support", "org.ashish.ablock")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "org.ashish.ablock")
		} else {
			return filepath.Join(home, ".org.ashish.ablock")
		}
	}
...
func handleXGenesisID(h http.Handler, genesisID ablock.Bytes32) http.Handler {
...
func selectGenesis(ctx *cli.Context) (*genesis.Genesis, ablock.ForkConfig, error) {
	network := ctx.String(networkFlag.Name)
	if network == "" {
		_ = cli.ShowAppHelp(ctx)
		return nil, ablock.ForkConfig{}, errors.New("network flag not specified")
	}

	switch network {
	case "test":
		gene := genesis.NewTestnet()
		return gene, ablock.GetForkConfig(gene.ID()), nil
	case "main":
	...
	default:
		...
		if err != nil {
			return nil, ablock.ForkConfig{}, errors.Wrap(err, "open genesis file")
		}
		defer file.Close()
		...
		var forkConfig = ablock.NoFork
		var gen genesis.CustomGenesis
		gen.ForkConfig = &forkConfig

		if err := decoder.Decode(&gen); err != nil {
			return nil, ablock.ForkConfig{}, errors.Wrap(err, "decode genesis file")
		}

		customGen, err := genesis.NewCustomNet(&gen)
		if err != nil {
			return nil, ablock.ForkConfig{}, errors.Wrap(err, "build genesis")
		}
...
func beneficiary(ctx *cli.Context) (*ablock.Address, error) {
	value := ctx.String(beneficiaryFlag.Name)
	if value == "" {
		return nil, nil
	}
	addr, err := ablock.ParseAddress(value)
	if err != nil {
...
func newP2PComm(ctx *cli.Context, repo *chain.Repository, txPool *txpool.TxPool, instanceDir string) (*p2pComm, error) {
	...
	opts := &p2psrv.Options{
		Name:            common.MakeName("ablock", fullVersion()),
		PrivateKey:      key,
...
func startAPIServer(ctx *cli.Context, handler http.Handler, genesisID ablock.Bytes32) (string, func(), error) {
...
func printStartupMessage1(
    ...
    Instance dir [ %v ]
`,
		common.MakeName("Ablock", fullVersion()),
		gene.ID(), gene.Name(),
...
└──────────────────┴───────────────────────────────────────────────────────────────────────────────┘
`,
		common.MakeName("Ablock solo", fullVersion()),
		gene.ID(), gene.Name(),
		bestBlock.Header.ID(), bestBlock.Header.Number(), time.Unix(int64(bestBlock.Header.Timestamp()), 0),
		forkConfig,
		dataDir,
