...
func verifyLogDB(ctx context.Context, endBlockNum uint32, repo *chain.Repository, logDB *logdb.LogDB) error {
	...
	var (
		...
		logLimit    = uint32(0)
		splitEvLogs = func(id ablock.Bytes32) (logs []*logdb.Event) {
			if len(evLogs) == 0 {
				return
			}
		...
		splitTrLogs = func(id ablock.Bytes32) (logs []*logdb.Transfer) {
			if len(trLogs) == 0 {
				return
			}
...
func verifyLogDBPerBlock(
	...
	convertTopics := func(topics []ablock.Bytes32) (r [5]*ablock.Bytes32) {
...
func pumpBlockAndReceipts(ctx context.Context, repo *chain.Repository, headID ablock.Bytes32, from, to uint32, ch chan<- *block.Block) error {
	var (
