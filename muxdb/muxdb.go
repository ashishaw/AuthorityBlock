...
import (
	...
	"github.com/ashishaw/authorityblock/muxdb/internal/engine"
	"github.com/ashishaw/authorityblock/muxdb/internal/trie"
...
// initially empty.
func (db *MuxDB) NewTrie(name string, root ablock.Bytes32, commitNum, distinctNum uint32) *Trie {
	return trie.New(
		db.trieBackend,
		name,
		root,
...
// If root is zero or blake2b hash of an empty string, the trie is
// initially empty.
func (db *MuxDB) NewNonCryptoTrie(name string, root ablock.Bytes32, commitNum, distinctNum uint32) *Trie {
	return trie.New(
		db.trieBackend,
		name,
		root,
