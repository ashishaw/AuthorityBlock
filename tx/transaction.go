...
import (
	...
	"github.com/ashishaw/authorityblock/metric"
...
type body struct {
	...
	GasPriceCoef uint8
	Gas          uint64
	DependsOn    *ablock.Bytes32 `rlp:"nil"`
	Nonce        uint64
	Reserved     reserved
	Signature    []byte
}
...
func (t *Transaction) ID() (id ablock.Bytes32) {
	if cached := t.cache.id.Load(); cached != nil {
		return cached.(ablock.Bytes32)
	}
...
func (t *Transaction) EvaluateWork(origin ablock.Address) func(nonce uint64) *big.Int {
	hashWithoutNonce := ablock.Blake2bFn(func(w io.Writer) {
	...
	return func(nonce uint64) *big.Int {
		...
		hash := ablock.Blake2b(hashWithoutNonce[:], nonceBytes[:])
...
func (t *Transaction) SigningHash() (hash ablock.Bytes32) {
...
func (t *Transaction) DependsOn() *ablock.Bytes32 {
...
func (t *Transaction) Origin() (ablock.Address, error) {
	...
	origin := ablock.Address(crypto.PubkeyToAddress(*pub))
...
func (t *Transaction) DelegatorSigningHash(origin ablock.Address) (hash ablock.Bytes32) {
	return ablock.Blake2b(t.SigningHash().Bytes(), origin[:])
}

// Delegator returns delegator address who would like to pay for gas fee.
func (t *Transaction) Delegator() (*ablock.Address, error) {
	if err := t.validateSignatureLength(); err != nil {
		return nil, err
	}
	...
	delegator := ablock.Address(crypto.PubkeyToAddress(*pub))
...
func (t *Transaction) ProvedWork(headBlockNum uint32, getBlockID func(uint32) (ablock.Bytes32, error)) (*big.Int, error) {
	ref := t.BlockRef()
	refNum := ref.Number()
	...
	if delay := headBlockNum - refNum; delay > ablock.MaxTxWorkDelay {
		return &big.Int{}, nil
	}
...
func IntrinsicGas(clauses ...*Clause) (uint64, error) {
	if len(clauses) == 0 {
		return ablock.TxGas + ablock.ClauseGas, nil
	}

	var total = ablock.TxGas
	var overflow bool
	for _, c := range clauses {
		...
		if c.IsCreatingContract() {
			// contract creation
			cgas = ablock.ClauseGasContractCreation
		} else {
			cgas = ablock.ClauseGas
		}
