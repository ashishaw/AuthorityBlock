...
func TestTx(t *testing.T) {
	to, _ := ablock.ParseAddress("0x7567d83b7b8d80addcb281a71d54fc7b3364ffed")
	...
	assert.Equal(t, ablock.Bytes32{}, trx.ID())
...
func TestDelegatedTx(t *testing.T) {
	...
	hash := trx.DelegatorSigningHash(ablock.Address(o))
...
func TestIntrinsicGas(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, ablock.TxGas+ablock.ClauseGas, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(&ablock.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, ablock.TxGas+ablock.ClauseGas, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(nil))
	assert.Nil(t, err)
	assert.Equal(t, ablock.TxGas+ablock.ClauseGasContractCreation, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(&ablock.Address{}), tx.NewClause(&ablock.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, ablock.TxGas+ablock.ClauseGas*2, gas)
}

func BenchmarkTxMining(b *testing.B) {
	tx := new(tx.Builder).Build()
	signer := ablock.BytesToAddress([]byte("acc1"))
	maxWork := &big.Int{}
